...
-export([bind_q/2, unbind_q/2
         ,directory_resp/1, directory_resp_v/1
         ,api_request/1, api_request_v/1
         ,api_resp_v/1
         ,bgapi_request/1, bgapi_request_v/1
         ,bgapi_resp_v/1
         ,ping_request/1, ping_request_v/1
         ,ping_resp_v/1
         ,sendmsg_request/1, sendmsg_request_v/1
         ,sendmsg_resp_v/1
         ,declare_exchanges/0
...
directory_resp(Prop) when is_list(Prop) ->
    case directory_resp_v(Prop) of
        'true' -> kz_api:build_message(Prop, ?DIRECTORY_RESP_HEADERS, ?OPTIONAL_DIRECTORY_RESP_HEADERS);
        'false' -> {'error', "Proplist failed validation for directory_resp"}
    end;
directory_resp(JObj) -> directory_resp(kz_json:to_proplist(JObj)).

-spec directory_resp_v(kz_term:api_terms()) -> boolean().
directory_resp_v(Prop) when is_list(Prop) ->
    Valid = kz_api:validate(Prop, ?DIRECTORY_RESP_HEADERS, ?DIRECTORY_RESP_VALUES, ?DIRECTORY_RESP_TYPES),
    Valid;
directory_resp_v(JObj) -> directory_resp_v(kz_json:to_proplist(JObj)).
...
api_request(JObj) -> api_request(kz_json:to_proplist(JObj)).
...
api_request_v(Prop) when is_list(Prop) ->
    ...
    Valid;
api_request_v(JObj) -> api_request_v(kz_json:to_proplist(JObj)).
...
api_resp_v(Prop) when is_list(Prop) ->
    ...
    Valid;
api_resp_v(JObj) -> api_resp_v(kz_json:to_proplist(JObj)).
...
bgapi_request(Prop) when is_list(Prop) ->
    case bgapi_request_v(Prop) of
        'true' -> kz_api:build_message(Prop, ?BGAPI_REQUEST_HEADERS, ?OPTIONAL_BGAPI_REQUEST_HEADERS);
        'false' -> {'error', "Proplist failed validation for bgapi_request"}
    end;
bgapi_request(JObj) -> bgapi_request(kz_json:to_proplist(JObj)).

-spec bgapi_request_v(kz_term:api_terms()) -> boolean().
bgapi_request_v(Prop) when is_list(Prop) ->
    Valid = kz_api:validate(Prop, ?BGAPI_REQUEST_HEADERS, ?BGAPI_REQUEST_VALUES, ?BGAPI_REQUEST_TYPES),
    Valid;
bgapi_request_v(JObj) -> api_request_v(kz_json:to_proplist(JObj)).


-spec bgapi_resp_v(kz_term:api_terms()) -> boolean().
bgapi_resp_v(Prop) when is_list(Prop) ->
    Valid = kz_api:validate(Prop, ?BGAPI_RESPONSE_HEADERS, ?BGAPI_RESPONSE_VALUES, ?BGAPI_RESPONSE_TYPES),
    Valid;
bgapi_resp_v(JObj) -> bgapi_resp_v(kz_json:to_proplist(JObj)).
...
ping_request(JObj) -> ping_request(kz_json:to_proplist(JObj)).
...
ping_request_v(Prop) when is_list(Prop) ->
    ...
    Valid;
ping_request_v(JObj) -> api_request_v(kz_json:to_proplist(JObj)).

-spec ping_resp_v(kz_term:api_terms()) -> boolean().
ping_resp_v(Prop) when is_list(Prop) ->
    Valid = kz_api:validate(Prop, ?PING_RESPONSE_HEADERS, ?PING_RESPONSE_VALUES, ?PING_RESPONSE_TYPES),
    Valid;
ping_resp_v(JObj) -> ping_resp_v(kz_json:to_proplist(JObj)).
...
sendmsg_request(Prop) when is_list(Prop) ->
    case sendmsg_request_v(Prop) of
        'true' -> kz_api:build_message(Prop, ?SENDMSG_REQUEST_HEADERS, ?OPTIONAL_SENDMSG_REQUEST_HEADERS);
        'false' -> {'error', "Proplist failed validation for sendmsg_request"}
    end;
sendmsg_request(JObj) -> sendmsg_request(kz_json:to_proplist(JObj)).

-spec sendmsg_request_v(kz_term:api_terms()) -> boolean().
sendmsg_request_v(Prop) when is_list(Prop) ->
    Valid = kz_api:validate(Prop, ?SENDMSG_REQUEST_HEADERS, ?SENDMSG_REQUEST_VALUES, ?SENDMSG_REQUEST_TYPES),
    Valid;
sendmsg_request_v(JObj) -> sendmsg_request_v(kz_json:to_proplist(JObj)).

-spec sendmsg_resp_v(kz_term:api_terms()) -> boolean().
sendmsg_resp_v(Prop) when is_list(Prop) ->
    Valid = kz_api:validate(Prop, ?SENDMSG_RESPONSE_HEADERS, ?SENDMSG_RESPONSE_VALUES, ?SENDMSG_RESPONSE_TYPES),
    Valid;
sendmsg_resp_v(JObj) -> sendmsg_resp_v(kz_json:to_proplist(JObj)).
...
publish_fetch_resp(Id, Section, Resp) ->
    {'ok', Payload} = kz_api:prepare_api_payload(Resp, ?DIRECTORY_RESP_VALUES, fun directory_resp/1),
    kz_amqp_util:basic_publish(?FREESWITCH_EXCHANGE, <<"KAZOO.", (kz_term:to_binary(Section))/binary, ".response.", Id/binary>>, Payload, ?DEFAULT_CONTENT_TYPE).
...
publish_api_request(Request) ->
    ...
    kz_amqp_util:basic_publish(?FREESWITCH_EXCHANGE, <<"KAZOO.command.api">>, Payload, ?DEFAULT_CONTENT_TYPE, [{'reply_to', ServerID}, {'correlation_id', MsgID}]).

-spec publish_bgapi_request(kz_term:api_terms()) -> 'ok'.
publish_bgapi_request(Request) ->
    ServerID = props:get_value(<<"Server-ID">>, Request),
    MsgID = props:get_value(<<"Msg-ID">>, Request),
    {'ok', Payload} = kz_api:prepare_api_payload(Request, ?BGAPI_REQUEST_VALUES, fun bgapi_request/1),
    kz_amqp_util:basic_publish(?FREESWITCH_EXCHANGE, <<"KAZOO.command.bgapi">>, Payload, ?DEFAULT_CONTENT_TYPE, [{'reply_to', ServerID}, {'correlation_id', MsgID}]).
...
publish_ping_request(Request) ->
    ...
    kz_amqp_util:basic_publish(?FREESWITCH_EXCHANGE, <<"KAZOO.command.ping">>, Payload, ?DEFAULT_CONTENT_TYPE, [{'reply_to', ServerID}, {'correlation_id', MsgID}]).

-spec publish_sendmsg_request(kz_term:api_terms()) -> 'ok'.
publish_sendmsg_request(Request) ->
    ServerID = props:get_value(<<"Server-ID">>, Request),
    MsgID = props:get_value(<<"Msg-ID">>, Request),
    {'ok', Payload} = kz_api:prepare_api_payload(Request, ?SENDMSG_REQUEST_VALUES, fun sendmsg_request/1),
    kz_amqp_util:basic_publish(?FREESWITCH_EXCHANGE, <<"KAZOO.command.sendmsg">>, Payload, ?DEFAULT_CONTENT_TYPE, [{'reply_to', ServerID}, {'correlation_id', MsgID}]).